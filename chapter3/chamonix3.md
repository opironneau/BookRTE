...
### Companion script of 2 articles published in JCP
	Golse, F., Hecht, F., Pironneau, O., Tournier, P. H., & Smets, D. (2022). Radiative Transfer 
	For Variable 3D Atmospheres.
...
The documentation is written for the MacOS; changes maybe needed for Windows and Linux.  The script uses freefem-mpi to run in parallel.

### About Htools
Htools (https://github.com/htools/htools) is designed for electromagnetism, accoustic, etc, by boundary element methods.  
Interface with Htools for a new problem, with a new kernel like here, can be done by defining and interfacing a Generator class deriving from the virtual class VirtualGenerator of Htool. This is done here in the freefem plugin "RadiativeTransfer_htool.cpp".

You need to compile this file by typing in a terminal window

ff-c++  -I/Users/pironneau/ff/ff-10/3rdparty/boost//include -I/Users/pironneau/ff/ff-10/plugin//mpi -auto RadiativeTransfer_htool.cpp

where "pironneau", my base folder, must be changed to yours.

### About RadiativeTransfer_htool.cpp
The "Generator" class contains the description of the kernel; it is createdt through a user-defined constructor, for example here  GenSE which takes for arguments a mesh3, a meshS, two vectors and a new user-defined type KappaGrid (also defined in the plugin); thence, to use it:
...
The Generator can then be passed to Htool through the Build() function, which returns the compressed H-Matrix built by Htool from the user-defined copy_submatrix (or get_coef) routine  of the Generator computing the matrix coefficients:
	...
	HMatrix HSE = Build(GenSE,VhS,Vh,eta=100,eps=1e-2,minclustersize=10,maxblocksize=1000000

### To launch this script: 
for exemple:

ff-mpirun -np 8 chamonix.edp -ns -wg -Kmax 1 -fullmesh -n 15

which means with 8 processors no script, with graphics with one level of $\kappa$ (grey case) and a mesh refinement defined by n=15
...
### Remark
You are reading this MarkDown file which contains explainations and the freefem script. Freefem selects the lines between the 3 tildes c++ or freefem and the 3 tildes. For a user friendly environment install the extension "vscode-FreeFEM" in the VSCode app. If your code is "not trusted" do Ctrl+Maj+P and look for "trust workspace settings".
...
~~~freefem
// NBPROC 8
// PARAM -ns -wg -n 8 -Kmax 9
load "msh3"  // for 3D meshes
load "bem"  // boundary element methods for electromagnetism (htools)
load "RadiativeTransfer_htool"  // contains the functions specific to this application programme in C++ ad R for reflexions
...
- The light source from the ground is 
  ...
  $$
- $\gamma$ is the angle of incidence of the light source (to study the difference between morning and evennng),
- $Q_0==2.10^{-5}$ is its intensity,  (to understand why the sun light comes from the ground, see the paper).
- $T_{sun}=1.209$ is the scale sun temperature (5800K before rescaling).
- $ (\beta+(1-\beta){\bf 1}_{z<s_{al}})$ is a factor to account for snow on the ground above altitude $s_{al}$.
- The scattering coefficient depends on the presence of a cloud, assumed to be in a band $x\in[1.5-\sqrt{0.5},1.5+\sqrt{0.5}]$ betwwen atitude $cloud_m$ and $cloud_M$.  It ispossible to have a random density/scattering in the cloud.
 Here the scattering coefficient is $a_{scat}{\bf 1}_{z\in(cloud_m,cloud_M)}{\bf 1}_{(x-1.5)^2<0.5}$.

### Algorithmic Parameters
- Newton = the maximum Newton iterations for the temperature equation
- heat: the temperature equation is the heat PDE with thermal diffusion
- Niter the the number of global iterations $T\to I\to T$.

### 

~~~freefem  
/********************* SCRIPT PARAMETERS *********************/
int dc = 1, 							// samples every dc points in the nu integrals (speedup with dc>1, danger, don't change)
	Newton = 40,						// max Newton iter for the tempe eq
	n = getARGV("-n", 8),				// controls the number of vertices in the mesh
	...
	altkappa = getARGV("-altkappa", 1);	// if 1 (resp-1) Kappa is changed in a range for CO2 (resp CH4)

real sal = 10*nosnow+0.25, beta=0.3, 	// snow altitude limit and snow albedo
	Tsurf=(273+18)/4798., Q0=3.; // Infrared from Earth
//	Tsurf=5800/4798., Q0=2.e-5, sigma=sqr(sqr(pi))/15; // the Sunlight
real ascat =0., cloudm=0.4, cloudM=0.7; // add ascar fuzzy scattering in altitude [zscalm,zscalM]
real  sigma=sqr(sqr(pi))/15; // 
~~~
### Names of I/O files

There are two input files and several output files.

One input file defines in $\texttt{kappafile}$  pointwise $\nu\mapsto\kappa_\nu$, the other defines $\texttt{Thf}$ the mesh and altitude of the ground (see readmeshS).

All output files path are with respect to the "basedir"; their names reflect the test case. Hence outfiles from case do not erase the outpiles of another case if the cases are different.

### 
~~~freefem
/********************* OUTPUT FILES *********************/
string basedir("chamonix-output/");
//string basedir("cube-output/");
...
The file which defines the mesh must be next to this .md file.  For the format of a surface mesh see the freefem manual. 
...
//	meshS Thf = readmeshS("chamonix_100K_surf.meshb"); // uncomment to use the original fine mesh ; you can download it at https://freefem.org/misc/chamonix_100K_surf.meshb
	...
	int nnz = fullmesh ? 20 : (zmax-zmin)/hx; // controls the vertical nb of layers
...
The finite element space of degree 1 on the volumic mesh (tetradra) is called Vh.
The finite element space of degree 1 on the surfacic mesh (trangles) is called VhS.
It contains all the boundaries of the Th, the ground, the vertical sides and the top $z=Z$.

### 

~~~freefem
//if (mpirank == 0) savemesh(Th3,"Th3.mesh");
//if (mpirank == 0) Th3 = readmesh3("Th3.mesh");
...
~~~
### 
The last block above is a piecewise constant interpolation of $\nu\mapsto\kappa_\nu$ in $K$ levels. $\texttt{kappaApprox[K]}$ is the value of the K$^{th}$ level.
- if $\texttt{kappaInt[K]=0}$ then there exists a $k$ such that $\kappa[k]\in\texttt{[kappaApprox[K-1],kappaApprox[K]}$.
- When needed $\kappa[k]$ can be approximated by $\texttt{kappaApprox[K-1]}$ where $K$ is such that $\texttt{[kappaApprox[K-1]<= kappa[k] < kappaApprox[K]}$.

Now the x-dependency of the absorption is defined. It is the product of the density $\texttt{rhof}$  and the change of density in the cloud.

The ${\bf scattering}$ coefficient is  defined below in $\texttt{scatf}$.
...
/*****************************************/
func rhof = 0.5*(1-0.7*z) * (1 + (Kmax > 1)) * (1 + cloud*xi*(z>cloudm)*(z<cloudM)*(sqr(x-1.5)<0.5));//(sqr(x-1.5)+sqr(y+1.5)<0.5)) );
...
func scatf = 0.1+ascat*(z>cloudm)*(z<cloudM)*(sqr(x-1.5)<0.5);
...
Some integrals on the surface are computed.  For instance $\texttt{seeface1}$ is a vector on the vertices i of ThSof an integral on ThS -computed with mass lumping- of the vertical component of the normal Ns.z + gamma*(Ns.x -..) times Q0S times the $P^1$- hat function associated to vertex i.
###
...
The rectangular matrix R allows to find where on surface S is a point given in the 3D FEM space.
$[I,J,K]$ refers to a compact definition of a matrix storing  the row, column indices  when $K(I,J)\neq 0$. onGamma contains the index of the surface vertex corresponding to the same vertex of index J[i] in the volume mesh.
...
### Computation of the H-matrices

For each level K there are 2 H-matrices, one called HVolume[K] and the other called HSE[K].
The  file $\texttt{RadiativeTransfer\_htoolR.dylib}$ containes the functions necessary for their constructions. 
As the elements contain an integral of $\kappa$ on a segment, there is an interpolatoin of $\kappa$ on a uniform square grid to speed up the computations (see the type KappaGrid).
In the end 
$$
SE[K][i] = \int_\Sigma ((x-x').n) exp(-\int_{(x,x')}{\bf 1}_{\kappa(\nu,x'')\approx \kappa_{approx}[K]}d x'')/|x-x'|^3 dx'|_{x=q^ni}
$$

Similarly
$$
HVolume[K][i] = \int_\Omega\kappa(nu,x') exp(-\int_{(x,x')}\kappa_{approx}[K]{\bf 1}_{\kappa(nu,x'')}d x'')/|x-x'|^2
$$
...
if (kappaInK[K]) {
	...
	if (mpirank == 0) cout<<"Building volumic matrix K= "<<K<<endl;
...
~~~freefem
	...
	// The method:
	// Let 4 pi J(nu,x) = int_om I dw = \int_∑(x') exp(-kappa(nu,x')|x'-x|)(angle(N,x'-x))^2Bnu(nu,Tsurf)d∑ 
	...
	real[int] BnuTsurf(Nnu);
	for (int k=0; k<Nnu-1 ;k++) BnuTsurf[k] = Bnu(nu[k],Tsurf); // aim is to speed up below
...
Again here 2 integrals are computed for each level $\kappa_{approx}[K]$ of $\kappa$.
Notice that all computations are done in parallel on chunks of global vectors like  res[], aux[]. Thence at some points the chunks need to me gathered to the global vectors, when used as input for Hmatrix vector multiplies.
	...
	for (int niter=0; niter<Niter; niter++){ // Nonlinear loop
		...
		if (kappaInK[K]) {
			...
			if ( (kappa[k]>kappaApprox[K]) && (kappa[k]<=kappaApprox[K-1]) ) {    
				...
				auxs += BnuTsurf[k]*dnu; // auxs = Q0*int_{kappa(nu)=K} Bnu(nu,Tsurf) dnu
			}
...
The problem is of the type $F(T)=0$ for each $x$.  The derivative $F'$ is computed as if it was the grey case, i.e. $(\sigma T^4)'=4\sigma T^3$. Hence
$$
	...
	if (Kmax == 9) {
		...
		for (int k=0; k<Nnu-dc-1; k+=dc) { // every dc points, can be changed
			...
			if (K > 0) {
				...
				mpiAllgatherv(chunkbuff, res[], mpiCommWorld, sizes, offsets);
				bux[] = HVolume[K]*res[];
				for [i,bi:chunkbuff] bi = bux[][i+ii] + BnuTsurf[k]*SE[K][][i+ii];
...
//	if (mpirank == 0) medit("J5",Th3,Jold[5]);
/*	if (mpirank == 0) medit("J0",Th3,Jold[0]);
	if (mpirank == 0) medit("J1",Th3,Jold[1]);
	if (mpirank == 0) medit("J2",Th3,Jold[2]);
	if (mpirank == 0) medit("J3",Th3,Jold[3]);
	if (mpirank == 0) medit("J4",Th3,Jold[4]);
	if (mpirank == 0) medit("J5",Th3,Jold[5]);
	if (mpirank == 0) medit("J6",Th3,Jold[6]);
	if (mpirank == 0) medit("J7",Th3,Jold[7]);
	if (mpirank == 0) medit("J8",Th3,Jold[8]);
*/}
...
if (mpirank == 0) {
	...
	for (real Z=bounds[4]; Z<bounds[5];Z+=0.01) {
		real Tx = 4798*T(bounds[0]+hx/2-0.05,0.1+bounds[2]+hy/2,Z)-273;
		XX[kk] = Z; YY[kk++] = Tx;
		cout << Z << "  " << Tx << endl;
		myfile << Z << "  " << Tx << endl;
	}
...
~~~
  2 files in the vtk format is written for $T$
  - One for $T$ in Celcius and the volumic mesh
-  The other for $T$ in Celcius and the surface mesh
~~~freefem

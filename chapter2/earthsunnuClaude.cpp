...
//
// kappa file needs be on desktop
...
int updateJ(){
    ...
    for(int jnu=0; jnu<jmax; jnu++){  // frequency loop
        ...
        for(int i=0;i<Nz;i++) { // main altitude loop
            double z=i*dz;
            double J0z  = bearthnu*expint_E3(kappanuj*scloud(0,z))/2
                        + bsunnu*exp(-kappanuj*scloud(z,Z)/mus)/2
                        + c0*expint_E2(kappanuj*scloud(0,z))/2;
            for(int j=1;j<Nz;j++){ // convolution loop
                double zp=j*dz, z=i*dz, Sj4=(S[j]+S[j-1])*dz/4;
                if(zp>z)
                    J0z += expint_E1(kappanuj*scloud(zp,z-0.5*dz)) * Sj4;
                else
                    J0z += expint_E1(kappanuj*scloud(z,zp-0.5*dz)) * Sj4;
            }
            J0[jnu][i]=J0z;
        }
    }
...
int multiBlock(double initT)
{
    ...
    for(int k=0;k<kmax; k++){  // fixed point loop: first update F
        ...
        double normG=0;
        for(int j=0; j<jmax;j++)
            for(int i=0;i<Nz;i++){ normG+=fabs(J0[j][i]); }

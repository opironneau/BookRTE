...
const double Te=(273+18)/T0, Ts=5798/T0;// Earth andSun
...
const int Nz=80; // nb points in z
...
int updateJK(){
    ...
    for(int jnu=0; jnu<jmax; jnu++){  // frequency loop
        ...
        for(int i=0;i<Nz;i++){ // initial altitude loop;
            ...
            c0 -= q0* expint_E2(kappasum[jnu][i]*kappanuj)*J0old[jnu][i]*dz;
        }
